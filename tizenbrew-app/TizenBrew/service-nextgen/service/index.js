...
module.exports.onStart = function () {
  ...
  const app = express();
  let deviceIP;
  const isTizen3 = tizen.systeminfo
  ...
  app.all("*", (req, res) => {
    if (req.url.startsWith("/module/")) {
      ...
      fetch(
        `https://cdn.jsdelivr.net/${moduleName}/${req.url.replace(`/module/${encodedModuleName}/`, "")}`,
      )
        .then((fetchRes) => {
        ...
        .then(() => {
          ...
          res.type(
            path
              .basename(req.url.replace(`/module/${encodedModuleName}/`, ""))
              .split(".")
              .slice(-1)[0]
              .split("?")[0],
          );
  ...
  fetch("http://127.0.0.1:8001/api/v2/")
    .then((res) => res.json())
    .then((json) => {
      canLaunchInDebug =
        (json.device.developerIP === "127.0.0.1" ||
          json.device.developerIP === "1.0.0.127") &&
  ...
  function createAdbConnection(ip, mdl) {
    ...
    adbClient._stream.on("connect", () => {
      ...
      const shellCmd = adbClient.createStream(
        `shell:0 debug ${tbPackageId}.AxoBrew${isTizen3 ? " 0" : ""}`,
      );
  ...
  wsServer.on("connection", (ws) => {
    ...
    ws.on("message", (message) => {
      ...
      try {
        ...
        return wsConn.send(
          wsConn.Event(Events.Error, `Invalid JSON: ${message}`),
        );
      }
      ...
      switch (type) {
        case Events.AppControlData: {
          ...
          if (!module) {
            return wsConn.send(
              wsConn.Event(Events.Error, "App Control module not found."),
            );
          }
        ...
        case Events.LaunchModule: {
          ...
          if (mdl.packageType === "app") {
            ...
            currentModule.evaluateScriptOnDocumentStart =
              mdl.evaluateScriptOnDocumentStart;
          }
