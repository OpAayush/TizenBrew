    ...
    switch (type) {
      ...
      case Events.CanLaunchModules: {
        ...
        if (!this.modulesLoaded) {
          ...
          // Add delay to ensure modules are fully initialized
          setTimeout(() => {
    ...
    if (payload) {
      if (payload.type === "autolaunch" && !window.shouldDisableAutoLaunch) {
        const module = this.modules.find(
          (mdl) => mdl.fullName === payload.module,
        );
        if (!module) {
          this.context.dispatch({
            type: "SET_ERROR",
        ...
        // Add delay before navigation to ensure module is sent to service
        setTimeout(() => {
  ...
  processPendingEvents() {
    while (this.pendingEvents.length > 0) {
      ...
      if (event.type === Events.CanLaunchModules) {
        // Add delay for each pending event too
        setTimeout(() => {
